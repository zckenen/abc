define(['global', 'pagination', 'areaLinkpage'], function(Global, Pagination, AreaLinkpage){
	...
	var view = Backbone.View.extend({
		...
		list: function(pageNo,param) {
			...
			progress.start('L');
			// 获取数据
			self.collection.url = Global.path + '/a/per/admWorker/list';
		...
		render: function() {
			...
			this.collection.models.forEach(function(item, i){
				...
				model.index = ++i;
				content += template( 'placeWorkerListTpl', model );
			})
			// 插入渲染容器中
		...
		del: function( event ) {
			...
			// 确认是否删除
			UI.confirm('', function(){
				self.collection.url = Global.path + '/a/per/admWorker/delete';
	...
	var Add = Backbone.View.extend({
		...
		submit: function(){
			...
			self.setFormData();
			// 设置URL
			collection.url = Global.path + '/a/per/admWorker/save';
			// 写入数据
	...
	var Edit = Backbone.View.extend({
		...
		render: function( id ) {
			...
			// 渲染
			this.$el.find('.modal-body').html( template('placeWrkEditTpl', this.model.toJSON()) );
			// 数据绑定
			this.addFormData();
			// 数据验证
			this.validate();
